...
Speedup at $N=20$ & 1$\times$ (baseline) & \textbf{353$\times$}$^\dagger$ \\
...
We compared STP-Diff against an explicit STP estimate across network sizes $N \in \{3, 5, 7, 10,$ $15, 20\}$. Both implementations used identical Boolean rules for a randomly generated 10-node network. Timing was averaged over ten forward-backward passes on an Apple M4 Max.
...
\caption{Runtime comparison at $N=20$. STP-Diff achieves an estimated 353$\times$ speedup over explicit STP (see \S\ref{sec:scaling} for discussion).}
\label{fig:speedup}
\end{figure}
...
\textbf{Semi-Tensor Product Methods.} Cheng et al.~\cite{cheng2011analysis} established the algebraic framework for STP-based BN analysis. Cheng and Qi~\cite{cheng2010controllability} derived controllability conditions for STP Boolean networks, showing theoretical pathways for network intervention. Cheng and Zhang~\cite{cheng2025semi} recently extended STP to convolutional neural networks, but their method does not address the matrix explosion problem.
...
\textbf{Empirical vs. theoretical speedup.} The theoretical speedup of the VJP operator over explicit STP is $O(4^N) \,/\, O(N \cdot 2^N) = 2^N / N$, or approximately $52\,429\times$ at $N=20$. The measured $353\times$ speedup (Table~\ref{tab:complexity}) is an estimate against an idealized baseline, since explicit STP requires $O(4^N)$ memory and cannot be run at $N=20$ (the $2^N \times 2^N$ transition matrix would require 4~TB). The gap between $353\times$ and the theoretical $52\,429\times$ arises because the VJP forward pass executes $N-1$ sequential Kronecker products ($\otimes$) plus $N$ matrix-vector products in separate ``torch'' operations; GPU kernel launch overhead dominates on small tensors (the $2^N$ state vector is only 8~MB at $N=20$). This gap is expected to narrow at larger $N$ where tensor operations saturate the GPU, approaching the asymptotic bound.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Estimated 353$\times$ speedup} over explicit STP methods at $N=20$;
