...
Speedup at $N=15$ & 1$\times$ (baseline) & \textbf{$2\,563\times$}$^\dagger$ \\
...
\caption{Runtime comparison. STP-Diff achieves $2\,563\times$ measured speedup over explicit STP at $N=15$, with explicit STP becoming infeasible at $N \geq 20$ (see \S\ref{sec:scaling} for discussion).}
\label{fig:speedup}
\end{figure}
...
\textbf{Empirical vs. theoretical speedup.} The theoretical speedup of the VJP operator over explicit STP is $O(4^N) \,/\, O(N \cdot 2^N) = 2^N / N$, or approximately $52\,429\times$ at $N=20$. Our benchmark measures $2\,563\times$ at $N=15$ (Table~\ref{tab:complexity}), the largest size where the explicit $2^N \times 2^N$ transition matrix (1\,B elements, 4\,GB) can be constructed. At $N=20$, the explicit matrix would require 4~TB and cannot be built. The measured speedup is dominated by the cost of constructing the explicit matrix column-by-column ($\sim 58$\,s at $N=15$), a fundamentally $O(4^N)$ operation. The implicit VJP never constructs this matrix, reducing complexity to $O(N \cdot 2^N)$. At $N=20$, the $2^N$ state vector (8~MB) is small enough that Python-level autograd overhead from the $N$ sequential Kronecker products is still significant; the gap between measured and theoretical bounds should narrow at larger $N$ as tensor operations saturate available compute.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Up to $2\,563\times$ measured speedup} over explicit STP methods ($2\,563\times$ at $N=15$, theoretical bound $52\,429\times$ at $N=20$);
