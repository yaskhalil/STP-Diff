...
Speedup at $N=15$ & 1$\times$ (baseline) & \textbf{71$\times$}$^\dagger$ \\
...
\caption{Runtime comparison. STP-Diff achieves 71$\times$ measured speedup over explicit STP at $N=15$, with explicit STP becoming infeasible at $N \geq 20$ (see \S\ref{sec:scaling} for discussion).}
\label{fig:speedup}
\end{figure}
...
\textbf{Empirical vs. theoretical speedup.} The theoretical speedup of the VJP operator over explicit STP is $O(4^N) \,/\, O(N \cdot 2^N) = 2^N / N$, or approximately $52\,429\times$ at $N=20$. Our benchmark measures $71\times$ at $N=15$ (Table~\ref{tab:complexity}), the largest size where the explicit $2^N \times 2^N$ transition matrix (1\,B elements, 4\,GB) fits in memory. At $N=20$, the explicit matrix would require 4~TB and cannot be constructed. The gap between the $71\times$ measurement and the theoretical bound arises because the VJP forward pass executes $N-1$ sequential Kronecker products ($\otimes$) plus $N$ matrix-vector products in separate ``torch'' operations; Python-level autograd overhead and GPU kernel launch costs dominate at smaller tensor sizes. At $N=20$, the $2^N$ state vector (8~MB) is still small enough that launch overhead is significant; the gap should narrow at larger $N$ as the tensor operations saturate available compute.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Up to 71$\times$ measured speedup} over explicit STP methods (71$\times$ at $N=15$, theoretical bound $52\,429\times$ at $N=20$);
