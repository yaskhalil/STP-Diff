...
\textbf{Empirical vs. theoretical speedup.} The theoretical speedup of the VJP operator over explicit STP is $O(4^N) \,/\, O(N \cdot 2^N) = 2^N / N$, or approximately $52\,429\times$ at $N=20$. Our benchmark measures $1\,637\times$ at $N=15$ (Table~\ref{tab:complexity}), the largest size where the explicit $2^N \times 2^N$ transition matrix (1\,B elements, 4\,GB) can be constructed. At $N=20$, the explicit matrix would require 4~TB and cannot be built. The measured speedup is dominated by the cost of constructing the explicit matrix column-by-column ($\sim 34$\,s at $N=15$), a fundamentally $O(4^N)$ operation. The implicit VJP never constructs this matrix, reducing complexity to $O(N \cdot 2^N)$.

The speedup is slightly non-monotonic at intermediate sizes ($107\times$ at $N=10$, $92\times$ at $N=12$, $1\,637\times$ at $N=15$). The dip occurs because the VJP backward pass executes $N$ iterations, each building $\bigotimes_{j\neq i} y_j$ via a Python Kronecker-product loop. Between $N=10$ and $N=12$, this Python-level overhead grows faster ($\sim 1.8\times$ in loop iterations) than the explicit column-construction cost ($\sim 4\times$ in columns but each column's Kronecker chain shortens), temporarily reducing the ratio. By $N=15$, the $O(4^N)$ scaling of column construction more than compensates. A vectorized or compiled VJP (e.g., via \texttt{torch.compile}) would eliminate this overhead and approach the theoretical bound.
...
\paragraph{Author Contributions.} \textbf{Yaseen Khalil} conceived the core methodology, developed the implicit VJP operator and the differentiable STP framework, implemented all code and experimental scripts, conducted the computational experiments and benchmarking, performed the DepMap validation analysis, interpreted the results, and prepared the manuscript. \textbf{Pavel Kraikivski} and \textbf{Agnieszka Miedlar} jointly defined the project scope and research objectives, determined the biological datasets and validation strategy, supervised the research direction, and provided critical review, mentoring, and feedback throughout the project.

\paragraph{Data.} The authors thank the Broad Institute DepMap consortium (Public 25Q3) for public access to CRISPR-Cas9 knockout data.

\paragraph{Declaration.} AI-assisted language tools were used for formatting and prose refinement. All technical content, analysis, and conclusions remain the authors' original work.
