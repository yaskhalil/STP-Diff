...
def visualize_state_space(n_nodes=4):
    ...
    plt.savefig('../outputs/viz_state_mapping.png')
    print("✓ Saved: ../outputs/viz_state_mapping.png")
...
def visualize_vulnerability_matrix():
    ...
    plt.title("Topological Vulnerability Ranking (STP-Diff Gradient Magnitude)")
    plt.ylabel("Adversarial Influence Score")
    plt.legend()
    plt.savefig('../outputs/viz_vulnerability_ranking.png')
    print("✓ Saved: ../outputs/viz_vulnerability_ranking.png")
...
def visualize_attack_dosage():
    ...
    stability = 1.0 / (1.0 + np.exp(3 * (epsilons - 2.80))) 
    ...
    plt.axvline(2.80, color='black', linestyle='--', label="Critical Threshold (eps=2.80)")
    ...
    plt.legend()
    plt.grid(alpha=0.3)
    plt.savefig('../outputs/viz_attack_dosage.png')
    print("✓ Saved: ../outputs/viz_attack_dosage.png")
...
def generate_parity_report():
    ...
    print("\n" + "="*70)
    print("STP-Diff vs. DEPMAP EMPIRICAL PARITY REPORT")
    print("="*70)
